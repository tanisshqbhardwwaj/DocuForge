...
<div className="form-group item-rate-box">
  ...
  < section className = "form-section glass-card" >
        <h2 className="section-title">
          <i className="fas fa-calculator"></i> Totals
        </h2>
        <div className="form-grid cols-2">
          <div className="form-group">
            <label htmlFor="input-tax-rate">
              {user?.org_gst_registered ? "GST Rate (%)" : "Tax Rate (%)"}
            </label>
            <select
              id="input-tax-rate"
              className="form-input"
              value={formData.tax_rate}
              onChange={(e) =>
                onChange("tax_rate", parseFloat(e.target.value) || 0)
              }
            >
              <option value="0">
                0% {user?.org_gst_registered ? "(No GST)" : ""}
              </option>
              {user?.org_gst_registered ? (
                <>
                  <option value="5">5% (2.5% CGST + 2.5% SGST)</option>
          ...
          {!user?.org_gst_registered ? (
            ...
            >
              ...
              <input
                type="checkbox"
                checked={formData.show_tax_field}
                onChange={(e) => onChange("show_tax_field", e.target.checked)}
                style={{ width: "18px", height: "18px", cursor: "pointer" }}
              />
            </div>
        ...
        <div className="totals-summary">
          ...
          {formData.discount > 0 ? (
            <div className="total-row">
              ...
              <span>-{fmt((subtotal * formData.discount) / 100)}</span>
            </div>
          ) : null}
          {formData.tax_rate > 0 ? (
            user?.org_gst_registered ? (
              <>
                <div className="total-row">
                  <span>CGST ({formData.tax_rate / 2}%)</span>
                  <span>{fmt((subtotal * formData.tax_rate) / 200)}</span>
                </div>
                <div className="total-row">
                  <span>SGST ({formData.tax_rate / 2}%)</span>
                  <span>{fmt((subtotal * formData.tax_rate) / 200)}</span>
                </div>
              </>
            ) : formData.show_tax_field ? (
              <div className="total-row">
                <span>Tax ({formData.tax_rate}%)</span>
                <span>{fmt((subtotal * formData.tax_rate) / 100)}</span>
              </div>
            ) : null
          ) : null}
...
{/* Shipping & Logistics (Hide for PO as it's redundant) */ }
{
  formData.title !== "PURCHASE ORDER" && (
    <section className="form-section glass-card">
      <h2 className="section-title">
        <i className="fas fa-truck"></i> Shipping &amp; Logistics
      </h2>
      <div className="form-grid cols-2">
        <div className="form-group">
          <label htmlFor="input-po-number">P.O. Number (#)</label>
        ...
        <div className="form-group">
          <label htmlFor="input-shipping-date">Shipping Date</label>
          <input
            id="input-shipping-date"
            type="date"
            className="form-input"
            value={formData.shipping_date || ""}
            onChange={(e) => onChange("shipping_date", e.target.value)}
          />
        </div>
        <div className="form-group">
          <label htmlFor="input-transport-mode">Transport Mode</label>
        ...
        <div className="form-group">
          ...
          <input
            ...
            onChange={(e) => onChange("eway_bill", e.target.value)}
            placeholder="12-digit E-Way Bill #"
          />
        </div>
        <div className="form-group">
          ...
          <input
            ...
            onChange={(e) => onChange("sales_person", e.target.value)}
            placeholder="Name of Sales Person"
          />
        </div>
      </div>
            ...
            {formData.payment_method === "Online" ? (
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    onChange("transaction_id", e.target.value)
                  }
                  placeholder="Enter TXN ID or Ref No."
                />
              </div>
