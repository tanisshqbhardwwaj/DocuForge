export default function InvoiceForm({
  ...
  return (
    <div className="document-form" id="document-form">
      ...
      <section className="form-section glass-card">
        ...
        <div className="form-group" style={{ marginTop: '10px' }}>
          ...
          <input id="input-discount" type="text" className="form-input"
            ...
            onFocus={() => onFocusSection('totals')}
            onChange={onChange.bind(null, 'discount')} />
        </div>
      </section>
        ...
        <div className="totals-summary">
          ...
          {formData.tax_rate > 0 ? (
            user?.org_gst_registered ? (
              ...
              <div className="total-row"><span>Tax ({formData.tax_rate}%)</span><span>{fmt((subtotal - (subtotal * (formData.discount || 0) / 100)) * formData.tax_rate / 100)}</span></div>
            )
          ) : null}
      ...
      {/* Recurring Invoice */}
      <section className="form-section glass-card">
        <h2 className="section-title"><i className="fas fa-sync-alt"></i> Recurring Invoice</h2>
        <div className="form-group">
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <label style={{ margin: 0, fontWeight: '700', fontSize: '0.85rem' }}>Enable Recurring</label>
            <input type="checkbox" checked={formData.is_recurring || false}
        ...
        {formData.is_recurring && (
          <div className="form-grid cols-2" style={{ marginTop: '8px' }}>
            ...
            <div className="form-group">
              <label>No. of Occurrences</label>
              <input type="number" className="form-input" min="0"
                value={formData.recurrence_count || ''}
                onChange={e => onChange('recurrence_count', parseInt(e.target.value) || 0)}
                placeholder="0 = Unlimited" />
            </div>
            <div className="form-group">
              <label>Start Date</label>
              <input type="date" className="form-input"
                value={formData.recurrence_start_date || formData.date}
                onChange={e => onChange('recurrence_start_date', e.target.value)} />
            </div>
            <div className="form-group">
