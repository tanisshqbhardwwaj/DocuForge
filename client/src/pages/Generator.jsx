...
export default function Generator() {
  ...
  const handleAction = async (actionType) => {
    ...
    if (actionType === "email" && !formData.client_email) {
      showToast("Please enter a client email address first", "warning");
      return;
    }
    ...
    try {
      ...
      const docPayload = {
        ...formData,
        doc_type: formData.title.toLowerCase().includes("purchase")
          ? "purchase_order"
          : "invoice",
        subtotal,
      ...
      if (actionType === "email") {
        ...
        try {
          ...
          showToast(`Bill sent to ${formData.client_email}`, "success");
        } catch (mailErr) {
          console.error("Email failed:", mailErr);
          showToast("Failed to send email", "error");
        }
      } else {
        // 3. Generate PDF locally using Blob + Manual Download
        ...
        if (window.html2pdf) {
          ...
          a.href = url;
          const finalFilename = `${(formData.doc_number || "bill").replace(/[^a-z0-9]/gi, "_")}.pdf`;
      ...
      <div className="generator-header">
        <div>
          ...
          <p className="subtitle">
            Fill in the details and watch your document come to life
          </p>
        </div>
        <div style={{ display: "flex", gap: "10px" }}>
          ...
          >
            {saving ? (
              ...
              <>
                <i className="fas fa-file-download"></i> Download Bill
              </>
            )}
          </button>
        </div>
